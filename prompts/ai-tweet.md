...
| 1-b | 本文の中身が閲覧拒否・会員限定の誘導・ページのナビゲーション要素だけ | 文字数だけは 120 を超えるが記事の実質が無い（ルール #25）。§3-d で本文を読んだ時点で判定する。目印は `Access Denied`・`この記事は会員限定です`・`残り N 文字`・`ログインする`・サイト内メニューの羅列 |
...
- **2文目は記事の紹介で終わらせない（PM 2026-09-05 明示・ルール #24）**。「〜まで書かれた記事です」「〜が解説されています」のような紹介文は、読み手に「自分に関係がある」と思わせない。記事の中から **(1) それがどれだけ知られていないかを示す数値** **(2) なぜ気づけないのかの理由** のどちらかを選び、事実の断定として置く。読み手が自分の環境に当てはめて確認したくなる形が正解である。

  | | 例 |
  |---|---|
  | 弱い（紹介で終わる） | 原因の環境変数に触れた投稿がXに4件しかない点まで書かれた記事です |
  | 強い（PM 承認済み） | 原因の環境変数に触れた投稿はXに4件だけで、大半が気づかないまま使っています |

  行動を促す表現（要確認です・確認してください・必読）は §4-c のとおり禁止のままである。**指示ではなく事実の断定で同じ効果を出す**。
...
**PM 2026-09-05 承認**。2文目が「どれだけ知られていないか」の数値で終わっており、読み手が自分の環境を確認したくなる形になっている（ルール #24 の見本）。

```
8月18日以降、Claude Codeのパス指定ルールもサブディレクトリのCLAUDE.mdも読み込まれていません。原因の環境変数に触れた投稿はXに4件だけで、大半が気づかないまま使っています。
