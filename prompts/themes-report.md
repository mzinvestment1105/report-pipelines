...
7. **【最重要・PM 2026-05-23 確定】20 テーマ × WebSearch 並列実行（ローカル品質と同等担保）**：
   ...
   - 1 セット 5 本程度を並列発行
   - **「既存マクロ・国策情報で書ける場合は省略可」型の判断を絶対禁止**（PM 2026-05-23 確定・全 20 テーマで必ず WebSearch を実施する）。半導体・AI・量子等の頻出テーマでも省略しない
   - エラー時は別クエリ（クエリを変えて再試行）→ 別ソース（WebFetch 株探・みんかぶ）→ それでもダメな場合のみ「raw データの数値のみで動意理由は調査要」と明示・**テーマ自体をレポートから除外することは絶対禁止**（ランキング保全）
