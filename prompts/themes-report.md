...
7. **【最重要・PM 2026-05-23 確定】20 テーマ × WebSearch 並列実行（ローカル品質と同等担保）**：
   ...
   - **検索回数の上限（PM 2026-06-14 確定・リトライ暴走防止）**: WebSearch は **1 テーマ 1 本を厳守**（全 20 テーマ必須・省略禁止は維持）。エラー時のクエリ変更・リトライを含めても **合計 24 本を上限**とし、これを超えたテーマは raw データの数値のみで動意理由を記述する。「念のため複数回検索」「同テーマを角度を変えて再検索」での重複発行を禁止（1 テーマ 1 検索で得た情報で書き切る）
