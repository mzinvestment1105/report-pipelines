...
- **理由は必ず特定する（「理由不明」「需給主導」で逃げない・PM 2026-06-28 確定）**：raw（株探・Yahoo・掲示板スクレイプ済）の材料をまず使い、不足なら WebSearch / WebFetch（株探・みんかぶ・FISCO・決算短信・テーマ・「{銘柄コード} 株価 急騰 理由 {当月}」等）で補う。**新規の適時開示が無くても、当週の買い/売りを駆動している材料・業績・テーマのナラティブを特定して書く**。**EDINET の適時開示イベントだけで「材料なし」と判断しない**（小型急騰の理由は株探・掲示板・テーマ・決算に出る）。
