...
- **理由が parquet・raw・WebSearch（株探/みんかぶ/掲示板・「{銘柄コード} 株価 急騰 理由 {当月}」等）でも特定できない場合**：値動き描写で埋めず「当週は適時開示・ニュースを確認できず、理由は特定できていない」と正直に書く（必要なら値位置の文脈を一言添える）。推測補完・「需給主導」での逃げは禁止。
