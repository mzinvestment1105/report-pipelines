...
4. **取得失敗時**: 「掲示板・WebSearch・テーマ追跡すべて未取得」と明示・「需給主導」では逃げない
...
- 当該**項目（例：信用買残/発行済株数比）のみを需給ブロックから省略**する（他の取得済項目は記載）
- 銘柄自体は**絶対除外しない**（ランキングを保全）
- 銘柄エントリ内に「**この項目は raw・screening_master・WebFetch（株探・ヤフー・EDINET）すべてで取得失敗・調査要**」と 1 行明示する
- 「N/A」「不明」「データなし」と書くことは引き続き禁止
