...
5/27 動意レポートで「6613 QDレーザー -21.1%」の材料が **5/14 TDNet 7 連発と本日の増し担保規制入りを並列に書いただけ**で、**今日の -21.1% の直接原因が読み手に伝わらなかった**重大事故が発生したため確定。

### ❌ 絶対禁止のパターン（5/27 事故の実物）

```
材料: 2026-05-14 決算短信＋業績修正＋特別損失計上＋減資＋…の TDNet 7 件連発の継続評価。
...
- **背景材料**: 5/14 TDNet 7 件連発（決算短信＋業績下方修正＋特別損失計上＋減資＋
...
5/27 動意レポートで「8 位 141A トライアルホールディングス +0.1%（売買代金 98 億円）」の材料が **5/14 TDNet 既知材料の羅列 + 当日の値動き 1 行**のみで、**なぜ年初来安値圏で 98 億円もの資金が流入しているのか**が読み手に伝わらなかった事故への対処。

### ❌ 絶対禁止のパターン

```
材料: 2026-05-14 TDNet「通期連結業績予想の修正」「第 3 四半期決算短信」、5/11「月次売上速報」。
本日はザラ場で年初来安値更新後にプラス転引け。
```

→ 既知材料の羅列 + 当日値動きだけ。**98 億円の資金流入の文脈・流入主体推測がゼロ**。
...
```
**材料**:
- **今日のトリガー（テクニカル節目）**: 本日 2026-05-27 ザラ場で年初来安値（2,750 円）更新後にプラス転引け
  → 底値圏での切り返しで投機資金流入の可能性
- **背景材料**: 5/14 TDNet 通期業績修正＋第 3 四半期決算短信、5/11 月次売上速報（既知材料）

**資金流入の文脈（直近 3 ヶ月の株価動意ベース）**:
- **3 ヶ月のチャート要約**: 2/27 高値 4,200 円 → 5/27 安値 2,750 円（▲35% の下落基調）。
  現在 2,805 円・60 日レンジ下から 4% の底値圏。
- **売買代金の異常性**: 当日 98 億円は 5d 平均（推定 30 億円）の 3.3 倍。過去 3 ヶ月でも上位 5 日に入る規模。
- **流入主体の推測**: 年初来安値更新後の押し目買い派（長期投資家のリバランス買い・掲示板 TOPIX 採用思惑買い）と
  決算失望の利確売り（売買代金 10 位 5/25 の流れ）の交錯。機関大量保有報告 / 自社株買い等の構造材料は当日なし。
- **PM への示唆**: 押し目買い候補だが、安値圏ブレイクのリスクあり・2,600 円割れの戻り確認後の参入を推奨。
```
...
→ 並列で「今日の -21.1% の主因」が読み手に伝わらない（5/27 6613 QDレーザーで発生した実事故）
