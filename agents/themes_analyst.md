...
- **ETF/REIT/上場投信は構成銘柄として記載しない**（Public [prompts/_common_rules.md §1](../prompts/_common_rules.md)）
- **銘柄名 + コードのセット表記必須**（同 §7）
- テーマ名・事業モデル・業界用語・略語・固有サービス名に中学生レベルの括弧注釈（投資用語は注釈不要）。注釈の中で別のジャーゴンを使わない
- 時刻は和文・日本時間（冒頭で「※時刻は日本時間」を一度だけ明示）。英語見出し・原文転記禁止
