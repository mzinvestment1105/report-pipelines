...
**状態: 休止中（PM 2026-09-02）**。テーマ誌面の定期配信は廃止。テーマの読みは動意/PTS レポート内のテーマ欄（[theme_radar.py](../bi/pipelines/theme_radar.py) の自前ロジック）が担う。
...
3. 💼 関連度順 Top 5 銘柄リサーチ
   - **事業モデル**（Yahoo ファイナンスプロフィール由来・[prompts/_common_rules.md](../prompts/_common_rules.md) §5 の注釈を適用）
