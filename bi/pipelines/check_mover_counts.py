...
件数を機械カウントして、規定数に満たなければ exit 3 を返す（品質不合格の検知シグナル）。
exit code 規約（2026-07-07 統一）: 0=合格 / 1=インフラ失敗（ファイル読込不可等） /
3=品質ゲート不合格。
PM 2026-07-12 絶対配信原則: workflow は exit 3 でも送信を中止しない。exit 3 を検知したら
レポート冒頭へ品質注記を機械挿入して必ず配信し、⚠️「配信済み・品質注記あり」通知へ分岐する
（旧「exit 3 = ⛔ 送信中止＝未配信」設計は PM 却下・本スクリプトの exit code 自体は不変）。
...
def main() -> int:
    ...
    if failures:
        print("\n[件数ゲート] [NG] 品質ゲート不合格 (exit 3)。workflow が冒頭に品質注記を挿入して"
              "配信を続行します（PM 2026-07-12 絶対配信原則）:", file=sys.stderr)
        for f_ in failures:
            print(f"  - {f_}", file=sys.stderr)
        return 3  # 品質ゲート不合格（インフラ失敗 exit 1 と区別・注記配信分岐用の検知シグナル）
