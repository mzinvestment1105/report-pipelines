...
REQUIRED_SECTIONS = [
    # 番号有り・無し・絵文字付き全パターンをマッチ（claude-code-action は番号なしで生成することがある）
    (r"##\s*(?:\d+[.．]\s*)?[^\n]*市況スナップショット", "市況スナップショット",   "## 1. 市況スナップショット\n\n（要追記）\n"),
    (r"##\s*(?:\d+[.．]\s*)?[^\n]*今日の重要テーマ",     "今日の重要テーマ",       "## 2. 今日の重要テーマ\n\n（要追記）\n"),
    (r"##\s*(?:\d+[.．]\s*)?[^\n]*総合見通し",            "総合見通し",             "## 6. 総合見通し\n\n（要追記）\n"),
    # Deep Research 候補は廃止（2026-05-19 PM 確定・必須チェックから除外）
]
