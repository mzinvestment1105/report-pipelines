...
5. **同一材料の重複説明を絶対禁止**：1 材料（個別ニュース・企業・指標。例 Apple 値上げ・Oracle・ホルムズ海峡）の「何が起きたか／なぜ重要か」の説明は、最も適切な **1 セクションのみ**で行う。冒頭リード・市況スナップショット備考・先物動意理由・重要テーマで**同じ材料を説明し直さない**。他セクションでは 1 行ポインタか結論だけにする。リードは要点の列挙に留め材料の詳細説明をしない。**送信前に機械ゲートが同一材料の重複（同じ固有名詞が 4 セクション以上で説明）を検出し、基準超なら自動送信を保留する＝重複したレポートは配信されない**ため、必ず遵守すること
"""
