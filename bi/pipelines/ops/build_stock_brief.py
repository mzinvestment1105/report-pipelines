...
ALLOW_ANALYST: List[str] = [
    ...
    # --- page skeleton: the writer's single source of truth for headings/tables ---
    ...
    "ポジティブ",
    "ネガティブ",
    # NOTE: 「10. テクニカル」は 2026-09-08 に廃止し、以降の番号を繰り上げた
    #       （旧 11 リスク → 10 リスク・旧 12 目標株価・結論 → 11 目標株価・結論）。
    #       旧番号の項目を allowlist に残すと存在しない見出しとして毎回 WARNING が出て、
    #       本当の取りこぼしを見逃すため、現行の番号へ揃えた。
    "10. リスク（必須・最低3項目）",
    "11. 目標株価・結論",
    "廃止セクション（絶対出力禁止・PM 2026-05-30 確定）",
    # NOTE: 「❌ やってはいけないこと」「✅ 必ず守ること」「完了条件（Write 直前・
    #       簡易ゲート）」は 2026-09-07 の改訂で本文各節へ統合・削除された。
]

# NOTE: 意図的に除外した節（他レポート種別専用・執筆に不要・上位節と重複）:
#   §2/§2-C（動意の銘柄行）・§6（VIX）・§10（動意 raw）・§12（不可逆操作）・
#   §14（WebSearch の手順）・§15・§17・§19・§21/§21-A（マクロ市況）・
#   §22（マクロ）・§23/§23-A（イベントカレンダー）・§31（動意）・§33（送信形式）・
#   §35（夕方レポート）・§37（ファイル命名）・§38（テーマ2部）・付録A（grep 手順）
ALLOW_COMMON: List[str] = [
    # Kept: rules with no equivalent in Part 1, PLUS every section that Part 1
    # now points to instead of restating (§13/§25/§28/§29/§42/§43/§44/§47,
    # added 2026-09-07). A pointer in stock_analyst.md is only binding on the
    # writer if the pointed-to section is compiled into the brief, so any new
    # pointer must be paired with an entry here.
